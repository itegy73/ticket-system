        ...
        <div className="relative">
          <button
            ...
            className={`px-3 py-1.5 rounded-xl border flex items-center gap-2 transition-all cursor-pointer shadow-xs text-[11px] md:text-xs font-bold leading-none ${
              syncStatus.googleEmail && !googleUser?.isMock
          ...
          >
            ...
            <span>{syncStatus.googleEmail && !googleUser?.isMock ? 'جوجل شيت متصل ☁️' : 'ربط جوجل شيت ☁️'}</span>
          ...
          {showSheetsConfig && (
            <div className="absolute left-0 mt-2 w-80 origin-top-left rounded-xl bg-white dark:bg-[#1f1618] border border-gray-100 dark:border-maroon-900/40 p-4 shadow-xl z-50">
              <h3 className="font-bold text-sm text-gray-900 dark:text-gray-100 mb-1 flex items-center justify-between">
                ...
                <span className={`text-[10px] px-2 py-0.5 rounded-full ${syncStatus.googleEmail && !googleUser?.isMock ? 'bg-emerald-100 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-300' : 'bg-amber-100 dark:bg-amber-950/30 text-amber-800 dark:text-amber-400'}`}>
                  {syncStatus.googleEmail && !googleUser?.isMock ? 'مربوط كلياً' : 'وضع المعاينة بالخلفية'}
                </span>
              </h3>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-3 leading-relaxed">
                {syncStatus.googleEmail && !googleUser?.isMock 
                  ...
                  : 'يعرض التطبيق النسخة التفاعلية بالخلفية. اربط حسابك الفعلي ليقوم النظام تلقائياً بإنشاء ورقة البيانات الحقيقية باسمك على Drive لحفظها فورا.'}
              </p>
              ...
              {syncStatus.googleEmail && !googleUser?.isMock ? (
                <div className="mb-4 p-3 rounded-lg bg-gray-50 dark:bg-maroon-950/30 border border-gray-100 dark:border-maroon-900/20 space-y-2">
                  ...
                  <div className="pt-2 border-t border-gray-100 dark:border-maroon-900/10 flex flex-col gap-1">
                    <span className="text-[10px] text-gray-400">قواعد وجداول جوجل شيت السحابية:</span>
                    <button 
                      type="button"
                      onClick={() => {
                        setShowSheetsConfig(false);
                      ...
                      className="text-[11px] text-emerald-700 dark:text-emerald-400 hover:underline flex items-center gap-1 font-bold truncate cursor-pointer text-right w-full"
                    >
                      📊 فتح وبث ورقة العمل التفاعلية بالخلفية ↗
                    </button>
                  </div>
                ...
                <div className="mb-4 space-y-2">
                  {syncStatus.googleEmail && googleUser?.isMock && (
                    <div className="p-2.5 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-100 dark:border-amber-900/20 text-[11px] text-amber-800 dark:text-amber-300">
                      ⚠️ أنت حالياً في <strong>وضع محاكاة العرض التوضيحي</strong>. لتأسيس وحفظ ملف جوجل شيت حقيقي في حسابك على Google Drive، تفضل بتسجيل الدخول:
                    </div>
                  )}
                  <button 
                    type="button" 
                    ...
                    className="w-full h-10 flex items-center justify-center gap-2 px-3 py-2 border border-emerald-200 dark:border-emerald-900/40 rounded-lg hover:bg-emerald-50 dark:hover:bg-emerald-950/20 text-xs font-bold transition-all bg-white dark:bg-black text-emerald-700 dark:text-emerald-400 cursor-pointer shadow-xs"
                  >
                    ...
                    <span>تسجيل الدخول والربط الفعلي بحسابك</span>
                  </button>
                </div>
              )}
